...
json loadDictionary(string filePath) {
...
vector<string> partialMatch(json data, string substring) {
  ...
  for (auto &element : data.items()) {
    ...
    string key = element.key();   // words
    string val = element.value(); // definitions

    found = key.find(substring);

    if (found != string::npos && found == 0) { // if found >= 0 (its found then)
      matches.push_back(key);                  // add to matches
    }
  }
...
int main() {
  ...
  json dictionary = loadDictionary("./data/dictionary_small.json");

  // cout<<dictionary["avatar"]<<endl;
  // exit(0);
  ...
  while ((k = getch()) != 'Z') {
    ...
    if ((int)k != 32) { // if k is not a space print it
      ...
      for (auto &c : substr)
        c = tolower(c);
      // This is for setting the number of names that the program prints
      int end = min(10, (int)matches.size());
      // This prints out all found matches
      for (int i = 0; i < end; i++) {
        // word = turtle  subst=tle
        // loc = 3
        // word = turtle subst = tur
        // loc = 0
      ...
      if (matches.size() == 1) {
        ...
        // assuming matches[0] = "avatar"
        // then we could use matches[0] as a key to lookup a definition
        // dictionary[matches[0]]

        titleBar("Definition", console_size.width);
        cout << dictionary[matches[0]] << endl << endl;
      }
    }
  }
  return 0;
